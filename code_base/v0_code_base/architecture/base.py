...
class DecoderLLM(nn.Module):
    ...
    Responsibilities:
      ...
      - Optionally add image special tokens and expose their IDs
    ...
    def __init__(
        ...
        cfg : ModelConfig
            Per-model config.
        device_map : "auto" or dict
            Passed to AutoModelForCausalLM.from_pretrained.
        use_fast_tokenizer : bool
            Use HF fast tokenizer if available.
        add_image_special_tokens : bool
            If True, add <image_start>, <image_patch>, <image_end> to tokenizer.
        image_token_strings : dict, optional
            Custom strings for image special tokens, e.g.,
                {
        ...
        # Optional: add image special tokens
        self.image_token_ids: Dict[str, int] = {}
        if add_image_special_tokens:
            ...
            if image_token_strings is not None:
                default_tokens.update(image_token_strings)

            special_tokens_dict = {
                "additional_special_tokens": list(default_tokens.values())
            }
            num_added = self.tokenizer.add_special_tokens(special_tokens_dict)

            if num_added > 0:
                # Resize LLM embeddings to handle new tokens
                self.model.resize_token_embeddings(len(self.tokenizer))

            # Store IDs for easy use in Perceiver / projector code later
            self.image_token_ids = {
                name: self.tokenizer.convert_tokens_to_ids(tok)
                for name, tok in default_tokens.items()
            }
